...
internal static class WorkflowCommands
{
    ...
    public static void Add(RootCommand root, Command handoff, IMemoryWorkflowService service, Option<bool> json)
    {
        ...
        read.SetAction((p, ct) => Run(p, json, () => service.ReadAsync(Environment.CurrentDirectory, p.GetRequiredValue(readPath), p.GetRequiredValue(readFile), p.GetValue(section), ct)));
        ...
        update.SetAction((p, ct) => Run(p, json, () => service.SetSectionAsync(Environment.CurrentDirectory, p.GetRequiredValue(path), p.GetRequiredValue(file), p.GetRequiredValue(heading), p.GetRequiredValue(content), p.GetRequiredValue(hash), ct)));
        ...
        append.SetAction((p, ct) => Run(p, json, () => service.AppendAsync(Environment.CurrentDirectory, p.GetRequiredValue(appendPath), p.GetRequiredValue(appendFile), p.GetRequiredValue(appendContent), p.GetRequiredValue(appendHash), p.GetValue(supersedes), ct)));
        ...
        decisions.SetAction((p, ct) => Run(p, json, () => service.DecisionsAsync(Environment.CurrentDirectory, p.GetRequiredValue(decisionPath), p.GetValue(allDecisions), ct)));
        ...
        review.SetAction((p, ct) => Run(p, json, () => service.ReviewAsync(Environment.CurrentDirectory, p.GetRequiredValue(reviewPath), p.GetValue(reviewAfter), p.GetValue(status), p.GetRequiredValue(reviewHash), ct)));
        ...
        list.SetAction((p, ct) => Run(p, json, () => service.ListAsync(Environment.CurrentDirectory, p.GetValue(scope), p.GetValue(archived), ct)));
        ...
        attention.SetAction((p, ct) => Run(p, json, () => service.AttentionAsync(Environment.CurrentDirectory, p.GetValue(attentionScope), p.GetValue(days), ct)));
        ...
        context.SetAction((p, ct) => Run(p, json, () => service.ContextAsync(Environment.CurrentDirectory, p.GetRequiredValue(contextPath), p.GetValue(budget), p.GetValue(artifacts), ct)));
        ...
        resume.SetAction((p, ct) => Run(p, json, () => service.ResumeAsync(Environment.CurrentDirectory, p.GetRequiredValue(resumePath), p.GetValue(resumeBudget), ct)));
        ...
        handoffList.SetAction((p, ct) => Run(p, json, () => service.HandoffsAsync(Environment.CurrentDirectory, p.GetRequiredValue(handoffPath), ct)));
        ...
        show.SetAction((p, ct) => Run(p, json, () => service.ReadHandoffAsync(Environment.CurrentDirectory, p.GetRequiredValue(showPath), p.GetValue(handoffFile), ct)));
        ...
        doctor.SetAction((p, ct) => Run(p, json, () => service.DoctorAsync(Environment.CurrentDirectory, p.GetValue(repair), ct)));
        ...
        import.SetAction((p, ct) => Run(p, json, async () =>
        {
    ...
    private static async Task<int> Run<T>(System.CommandLine.ParseResult parse, Option<bool> json, Func<Task<T>> action)
    {
        try
        {
            var result = await action();
            Console.WriteLine(parse.GetValue(json) ? JsonSerializer.Serialize(result, JsonOptions) : Format(result));
        ...
        {
            Console.Error.WriteLine(parse.GetValue(json) ? JsonSerializer.Serialize(new { error = exception.Message }, JsonOptions) : $"Error: {exception.Message}");
